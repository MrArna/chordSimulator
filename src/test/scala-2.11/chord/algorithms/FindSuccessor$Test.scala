...
/**
  ...
  */
class FindSuccessor$Test extends TestKit(ActorSystem("FSTest",ConfigFactory.parseString(TestKitUsageSpec.config)))
  with DefaultTimeout
  with ImplicitSender
  with WordSpecLike
  with Matchers
  with BeforeAndAfterAll
{
  ...
  "The FS algorithm" should {
    "should return the successor" in {
              ...
              nodeObj2Fut onSuccess
                {
                  case result2: SetInstance => {
                    ...
                    assert(FindSuccessor.run(2, 7, nodesObj) == 2)
                  }
                }
            }
          }
