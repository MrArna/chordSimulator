...
/**
  ...
  */
class FindPredecessor$Test extends TestKit(ActorSystem("FPTest",ConfigFactory.parseString(TestKitUsageSpec.config)))
  with DefaultTimeout
  with ImplicitSender
  with WordSpecLike
  with Matchers
  with BeforeAndAfterAll
{
  ...
  "The FP algorithm" should {
    "should return the predecessor" in {
              ...
              nodeObj2Fut onSuccess
                {
                  case result2: SetInstance => {
                    ...
                    assert(FindPredecessor.run(2, 7, nodesObj) == 3)
                  }
                }
            }
          }
